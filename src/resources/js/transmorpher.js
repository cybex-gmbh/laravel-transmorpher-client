...
if (!window.transmorpherScriptLoaded) {
    ...
    window.setupComponent = function (transmorpherIdentifier) {
        ...
        const topic = topics[transmorpherIdentifier];
        ...
        if (topic.isProcessing || topic.isUploading) {
            ...
            setAgeElement(
                ...
                getDateForDisplay(new Date(topic.lastUpdated * 1000))
            );
        }
        ...
        new Dropzone(`#dz-${transmorpherIdentifier}`, {
            ...
            init: function () {
                ...
                this.on('processing', function () {
                    fetch(`${topic.routes.setUploadingState}/${this.options.uploadToken}`, {
                        method: 'POST',
                        headers: {
            ...
            canceled: function (file) {
                fetch(`${topics[transmorpherIdentifier].routes.handleUploadResponse}/${this.options.uploadToken}`, {
                    method: 'POST', headers: {
    ...
    window.handleUploadResponse = function (file, response, transmorpherIdentifier, uploadToken) {
        ...
        if (uploadToken) {
            fetch(`${topics[transmorpherIdentifier].routes.handleUploadResponse}/${uploadToken}`, {
                method: 'POST', headers: {
                    'Content-Type': 'application/json', 'X-XSRF-TOKEN': getCsrfToken(),
                }, body: JSON.stringify({
    ...
    window.updateVersionInformation = function (transmorpherIdentifier) {
        ...
        fetch(topics[transmorpherIdentifier].routes.getVersions, {
            ...
            Object.keys(versions).sort((a, b) => versions[b] - versions[a]).forEach(version => {
                ...
                switch (topics[transmorpherIdentifier].mediaType) {
                    case mediaTypes[IMAGE]:
                        versionEntry.querySelector('a').href = `${topics[transmorpherIdentifier].routes.getOriginal}/${version}`;
                        versionEntry.querySelector('.dz-image img:first-of-type').src = `${topics[transmorpherIdentifier].routes.getDerivativeForVersion}/${version}/w-150`;
                        versionEntry.querySelector('.dz-image img:first-of-type').srcset = `${topics[transmorpherIdentifier].routes.getDerivativeForVersion}/${version}/w-150 150w`;
                        break;
                    case mediaTypes[VIDEO]:
    ...
    window.reserveUploadSlot = function (transmorpherIdentifier, done) {
        ...
        fetch(topics[transmorpherIdentifier].routes.uploadToken, {
            ...
            dropzone.options.url = `${topics[transmorpherIdentifier].webUploadUrl}${getUploadTokenResult.upload_token}`;
