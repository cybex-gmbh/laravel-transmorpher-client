    ...
    <a class="full-size-link" target="_blank" href="{{ $motif->getUrl() }}">
        <div class="dz-image image-transmorpher">
            <img data-delivery-url="{{ $motif->getDeliveryUrl() }}"
                 ...
                 alt="{{ trans('transmorpher::image-alt-tags.preview_image', ['differentiator' => $differentiator]) }}"/>
            <img role="link" src="{{ mix('icons/enlargen.svg', 'vendor/transmorpher') }}" alt="{{ trans('transmorpher::image-alt-tags.show_image_in_full_size') }}" class="icon enlarge-icon">
        </div>
    </a>
</div>
